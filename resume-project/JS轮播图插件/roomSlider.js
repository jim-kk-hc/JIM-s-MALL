(function () {
	var RoomSlider = function (slideId) { //给一个参数传递 id
		...
		this.autoStart = setInterval(this.nextStart.bind(this), 1000); //定时器
...
// console.log()
		this.slide.setAttribute('class', 'slide');//给外层div +了一个class
	...
	RoomSlider.prototype.onmouseenter = function () {
		...
		this.down.style.display = 'block';	//next
		this.up.style.display = 'block'; //pre
	...
	RoomSlider.prototype.onmouseleave = function () {
		...
		clearInterval(this.autoStart);//为什么离开也要clearInterval
		this.autoStart = null;
	...
	RoomSlider.prototype.nextStart = function () {
		...
		this.time = setInterval(function () {
			...
			var left = parseInt(_this.room.style.left); //初始是0
			// console.log(getComputedStyle(_this.room).left)

			if (_this.imgIndex < _this.img.length) {//this.imgIndex 初始为一 
				//_this.img.length :5

				if (left > (-_this.slideWidth * _this.imgIndex)) {
					//_this.speed :-52
					_this.room.style.left = parseInt(_this.room.style.left) + _this.speed + 'px';
				} else {//当 left==-_this.slideWidth * _this.imgIndex
	...
	//相当于 window.RoomSlider=RoomSlider
})();